/* ─────────────────────────────────────────────
   Login.jsx — responsivo para desktop
   ───────────────────────────────────────────── */
...
function Login() {
    ...
    return (
        <>
            {/* ── Wrapper com layout de duas colunas no desktop ── */}
            <div className='bg-[#0A0A0A] min-h-screen overflow-x-hidden overflow-y-hidden relative flex items-center justify-center'>

                {/* Glows */}
                <div className='absolute z-0 -top-40 -right-20 w-125 h-125 bg-[#00FFFF]/70 rounded-full blur-[200px]' />
                <div className='absolute z-10 bottom-0 left-0 md:-bottom-120 md:-left-80 w-96 h-96 md:w-200 md:h-200 bg-[#AA00FF]/20 md:bg-[#AA00FF] rounded-full md:blur-[300px] blur-[200px]' />

                {/*
                  Mobile: coluna única centralizada
                  Desktop (lg+): duas colunas — esquerda com branding, direita com card
                */}
                <div className="relative z-10 w-full max-w-5xl px-6 flex flex-col lg:flex-row items-center justify-center gap-16">
                    ...
                    <div className="hidden lg:flex flex-col gap-8 flex-1 max-w-sm">
                        <div className="flex items-center gap-3">
                            <svg width="48" height="48" viewBox="0 0 43 42" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <span className="text-white text-3xl font-bold h-7 mt-0.5">ScraperCar</span>
                        </div>

                        <div>
                            <h2 className="text-white text-4xl font-bold leading-tight mb-4">
                                Seja o primeiro a<br/>
                                <span className="text-transparent bg-clip-text bg-linear-to-r from-[#00FFFF] to-[#AA00FF]">
                                    encontrar seu carro
                                </span>
                            </h2>
                            <p className="text-[#E0E0E0]/50 text-base leading-relaxed">
                                Crie alertas inteligentes e receba notificações no WhatsApp quando aparecerem veículos com o perfil que você procura.
                            </p>
                        </div>
                        ...
                        <div className="flex flex-col gap-3">
                            {[
                                { icon: '🔔', text: 'Notificações via WhatsApp' },
                                { icon: '🚗', text: 'Busca por marca, modelo e ano' },
                                { icon: '✅', text: 'Gerencie múltiplos alertas simultâneos' },
                            ].map(({ icon, text }) => (
                                <div key={text} className="flex items-center gap-3">
                                    <span className="text-xl">{icon}</span>
                                    <span className="text-[#E0E0E0]/70 text-sm">{text}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                            ...
                            >
                                <div className="p-7 pt-13">
                                    ...
                                    <form className="mt-10 grid" onSubmit={handleSubmit}>
                                        ...
                                        <button
                                            className="pt-3 h-11 justify-self-center text-white w-[50%] bg-[#AA00FF] rounded-full p-2 font-bold transition-all hover:shadow-[0_0_20px_rgba(170,0,255,0.5)] disabled:opacity-50 disabled:cursor-not-allowed"
                                            type="submit"
                                            disabled={isLoading}
                                        >
